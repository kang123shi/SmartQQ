...
public class SmartQQClient implements Closeable {
    ...
    private void getQRCode() {
        ...
        for (Cookie cookie : response.getCookies()) {
            if (TextUtils.equals(cookie.getName(), "qrsig")) {
                qrsig = cookie.getValue();
                break;
            }
        }
        logD("二维码已保存在 " + filePath + " 文件中，请打开手机QQ并扫描二维码");
...
//        .build();
    ...
    private static void checkSendMsgResult(Response<String> response) {
        if (response.getStatusCode() != 200) {
            logE(String.format("发送失败，Http返回码[%d]", response.getStatusCode()));
        }
        ...
        if (errCode != null && errCode == 0) {
            ...
            logE(String.format("发送失败，Api返回码[%d]", json.getInteger("retcode")));
        }
    }
